...
{laws && <div dangerouslySetInnerHTML={{__html: laws}} />}
            ...
            <form >
                <input style={{background:"#ddf0ff"}}  placeholder={`نام و نام خانوادگی: `} value={`نام و نام خانوادگی: ${find.usename} ${find.number[0].family}`} onChange={(e) => setusename(e.target.value)} />
                <input style={{background:"#ddf0ff"}} placeholder="شماره همراه" value={`شماره همراه: ${find.number[0].number}`} />
                <input style={{background:"#ddf0ff"}} placeholder={`کد ملی: `} value={`کد ملی: ${find.number[0].meliCode}`} />
                <input style={{background:"#ddf0ff"}} placeholder="ایمیل" value={`ایمیل: ${find?.email}`} />  
